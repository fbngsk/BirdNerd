...
export const Header: React.FC<HeaderProps> = ({ xp, locationStatus, isLoading, userProfile, isVacationMode, onToggleMode, onAvatarClick }) => {
    ...
    const themeColor = isVacationMode ? 'text-orange-600' : 'text-teal';
    ...
    return (
        <header className={`pt-8 pb-4 px-6 sticky top-0 z-20 shadow-sm border-b border-gray-200/50 transition-colors duration-500 ${isVacationMode ? 'bg-orange-50' : 'bg-cream'}`}>
            <div className="flex justify-between items-center mb-3">
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative cursor-pointer active:scale-95 transition-transform" onClick={onAvatarClick}>
                        ...
                        <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold border-2 border-white text-white ${isVacationMode ? 'bg-pink-500' : 'bg-teal'}`}>
                           {levelInfo.level}
                        </div>
                    </div>
